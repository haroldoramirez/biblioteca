...
public class TwitterComponentImpl implements TwitterComponent {
    ...
    @Inject
    public TwitterComponentImpl(ApplicationLifecycle lifecycle, Application application) {
        ...
        lifecycle.addStopHook( () -> {
            ...
            Logger.info("twitter plugin: stop");
    ...
    public List<Status> tweetsJson() {
        return tweets.value();
    }

    public List<Status> tweetsJson(int max) {
        List<Status> t = tweets.value();
        if (t.size() < max) {
            return t;
        } else {
